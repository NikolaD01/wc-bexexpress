    ...
    public function fields ($field, $key, $args, $value) : string
    {
        ...
        if ($key === 'billing_state') {
            if ($this->settings) {
                ...
                $field .= '<label for="' . esc_attr($key) . '" class="wfacp-form-control-label">' . 'Opština' . '&nbsp;<abbr class="required" title="required">*</abbr></label>';
                $field .= '<span class="woocommerce-input-wrapper">';
                // Change the name for the visible input
                $field .= '<input type="text" class="input-text wfacp-form-control" name="' . esc_attr($key) . '_display" id="' . esc_attr($key) . '_display" placeholder="' . esc_attr($args['placeholder'] ?? 'Enter state') . '" value="' . esc_attr($value) . '" />';
                $field .= '</span>';
        ...
        if ($key === 'billing_city') {
            if(!$this->settings) {
                ...
                $field .= '<label for="' . $key . '" class="wfacp-form-control-label">' . 'Mesto' . '&nbsp;<abbr class="required" title="required">*</abbr></label>';
                $field .= '<span class="woocommerce-input-wrapper">';
            ...
            else
            {
                ...
                $field .= '<label for="' . esc_attr($key) . '" class="wfacp-form-control-label">' . 'Mesto' . '&nbsp;<abbr class="required" title="required">*</abbr></label>';
                $field .= '<span class="woocommerce-input-wrapper">';
                $field .= '<input type="text" class="input-text wfacp-form-control" name="' . esc_attr($key) . '_display" id="' . esc_attr($key) . '_display" placeholder="' . esc_attr($args['placeholder'] ?? 'Enter state') . '" value="' . esc_attr($value) . '" />';
                $field .= '</span>';
                $field .= '<span class="wfacp_inline_error" data-key="' . esc_attr($key) . '_field"></span>';
        ...
        if ($key === 'billing_address_1') {
            if(!$this->settings) {
                $field = '<p class="form-row form-row-first address-field wfacp-form-control-wrapper wfacp-col-left-half wfacp_field_required validate-required" id="' . $key . '_field">';
                $field .= '<label for="' . $key . '" class="wfacp-form-control-label">' . 'Ulica' . '&nbsp;<abbr class="required" title="required">*</abbr></label>';
                $field .= '<span class="woocommerce-input-wrapper">';
                $field .= '<select name="' . $key . '" id="' . $key . '" class="input-text wfacp-form-control">';
            ...
            else
            {
                ...
                $field .= '<label for="' . esc_attr($key) . '" class="wfacp-form-control-label">' . 'Ulica' . '&nbsp;<abbr class="required" title="required">*</abbr></label>';
                $field .= '<span class="woocommerce-input-wrapper">';
